...
def parse_sets(session, gaze, task, run,
    ...
    sessions, gazes, tasks, runs = map(np.ravel, np.meshgrid(sessions, gazes, tasks, runs))
...
def get_data(subject=None, session=None, gaze=None, task=None, run=None,
    ...
    sessions, gazes, tasks, labels, runs = parse_sets(session, gaze, task, run, fullscreen)
    ...
    for session, gaze, task, label, run in pbar:
        pbar.set_description(f'Session {session}, Gaze {gaze}, Task {task}, run {run}')
...
def get_prf_parameters(subject, session=None, gaze='CenterFS', task=None, run=None,
        fullscreen=True,
        masker=None,
        bids_folder='/tank/shared/2021/visual/pRFgazeMod'):
    ...
    if (session is None) and fullscreen and (task is None) and (run is None):

        parameters = ['x', 'y', 'sd', 'baseline', 'amplitude']
        fs_pars = []
        ...
        for parameter in parameters:
            ...
            fs_pars.append(pd.DataFrame({parameter:p}, index=np.arange(len(p))))
